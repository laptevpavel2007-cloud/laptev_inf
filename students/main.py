...
cursor.execute('''
    SELECT фамилия, имя, отчество, средний_балл,
        CASE 
            WHEN средний_балл < 3 THEN 'Двоешник'
            WHEN средний_балл >= 3 AND средний_балл < 4 THEN 'Троешник'
            WHEN средний_балл >= 4 AND средний_балл < 4.5 THEN 'Хорошист'
            WHEN средний_балл >= 4.5 THEN 'Отличник'
        END as Успеваемость
    FROM студенты
''')
print("Ранги студентов:")
print(cursor.fetchall())
...
''')
print("Студенты со средним баллом выше средного значения:")
print(cursor.fetchall())

cursor.execute('''
    WITH
               отличники_студенты as (
               SELECT фамилия, имя, отчество FROM студенты WHERE средний_балл > 4.5
               ),
               РПО_отличники_студенты as (SELECT с.фамилия, с.имя, с.отчество, с.средний_балл 
               FROM студенты с 
               JOIN направления н ON с.id_направления = н.id_направления 
               JOIN типы_обучения т ON с.id_типа_обучения = т.id_типа 
               WHERE н.название = 'РПО')
               SELECT *
               FROM РПО_отличники_студенты;
''')
print("Студенты РПО отличники:")
print(cursor.fetchall())
